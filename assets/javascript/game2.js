...
//Arrays and Variables
	...
	//Create an empty array that will hold all the letters in the random word that the computer generates.
	var lettersInWord = [];

	//Number of underscores needed for random word.
	var numberUnderscoresNeeded = 0;

	//Create an empty array, which will be used to replace the underscores with successful guesses by the user.
...
function start () {
	...
	//Problem: After the computer generates a random word, need a way to add the appropriate number of underscores("_") to the Current Word section
	...
	document.onkeyup = function(event) {
			...
			//userGuess is our keyCode. Store the userGuess variable as a string. Also, if user has caps lock on, convert the letter from upper case to lower case.
...
function checkLetters(userGuess) {
	...
	for (var i = 0; i < numberUnderscoresNeeded; i++){
		...
		//Else, if letter that the user guessed was not found in the random word, isletterInWord is false.
	...
	//If the letter is in the word, check where in the word the letter exists, and then populate the underscoresSuccesses array in the HTML.
	if (isletterInWord) {
		 for (var i = 0; i < numberUnderscoresNeeded; i++) {
...
function roundComplete() {
	...
	//Convert the letters to strings in both arrays to compare.
	if (lettersInWord.toString() == underscoresSuccesses.toString()) {
		wins++;
		...
		//Create a confirm dialog box to ask the user if they want to play the game. If user clicks OK, reset game and choose new word.
	...
	//Check if user lost. If remaining guesses gets down to zero, user lost.
	else if (guessesRemainder === 0){
		losses++;
		...
		//Create a confirm dialog box to ask the user if they want to play the game again. If user clicks OK, reset game and choose new word.
		//If user clicks Cancel, go back to main home screen.
		var playAgain = confirm('Do you want to play again?');
		if (playAgain) {
			start();
...
//Execute the code. Do not place anything else below this line.
//===================================================================

//Starts/resets the code/game
start();

//check and compare the letter that the user guessed to the letters in the random word generated by the computer..
